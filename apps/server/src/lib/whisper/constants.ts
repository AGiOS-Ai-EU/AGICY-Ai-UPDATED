...
export function getModelsDir(): string {
  return join(homedir(), ".cache", "updated", "whisper-models");
}

/** Prefer UPDATED cache; fall back to legacy freestyle path if present. */
export function getModelPath(model: WhisperModelDef): string {
  const primary = join(getModelsDir(), model.fileName);
  const legacy = join(
    homedir(),
    ".cache",
    "freestyle",
...
export function getBinDir(): string {
  return join(homedir(), ".cache", "updated", "whisper-bin");
}
